...
BOT_TOKEN = "7791394191:AAG4XXCeubVvT7eU8O07MfDCP_UjfQ2ECUs"
thread_count = 50
...
def send_owner_info(message):
    owner_message = "This Bot Has Been Developed By @Mk_ddos"  
...
def send_welcome(message):
    ...
    if is_super_admin(user_id):
        welcome_message = (
            f"Welcome, Super Admin! To {_d(MONSTER)}\n\n"
            f"Admin Commands:\n"
...
def attack_button_handler(message):
    ...
    if not found_user:
        bot.send_message(chat_id, "*𝐘𝐨𝐮 𝐚𝐫𝐞 𝐧𝐨𝐭 𝐫𝐞𝐠𝐢𝐬𝐭𝐞𝐫𝐞𝐝. 𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- @Mk_ddos*", parse_mode='Markdown')
        return

    valid_until = datetime.fromisoformat(found_user['valid_until'])
    if datetime.now() > valid_until:
        bot.send_message(chat_id, "*𝐘𝐨𝐮𝐫 𝐤𝐞𝐲 𝐡𝐚𝐬 𝐞𝐱𝐩𝐢𝐫𝐞𝐝. 𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- @Mk_ddos.*", parse_mode='Markdown')
        return
    ...
    if is_super_admin(user_id):
        ...
        if datetime.now() > datetime.fromisoformat(found_user['valid_until']):
            account_info = (
                ...
                "Contact @Mk_ddos for assistance."
            )
        else:
            account_info = (
        ...
        account_info = "𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- @Mk_ddos."
